...
{
    s_fd = -1;
    ldConst.angle_min = -_M_PI + I_RAD / 16; //180+1/16 degree
...
int fslidar::getOnePoint(LaserDataNode *ldn, bool print)
{
    ...
    stringstream log;
#ifndef _FAKE
    recv_rtn = sPort.recv((char *)buf_rear, RECV_BUF_MAX); //1024Bytes
    //ROS_INFO("recv_rtn=%d",recv_rtn);
    if (recv_rtn <= 0)
    {
    ...
    buf_head = recv_buf;          //buf_head  and buf_rear is class membles
    buf_rear = recv_buf + i;        //buf_rear all raw_data rear
    return ldn_cnt;
